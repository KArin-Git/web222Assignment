...
function createVideo(src, width, controls) {
  let cleanScr = src.trim();
  let cleanWidth = Number(width);
  let result;

  if (controls) {
    if (cleanWidth > 1) {
      result = `<video src=\"${cleanScr}\" width=\"${cleanWidth}\" controls></video>`;
    } else {
      result = `<video src=\"${cleanScr}\" controls></video>`;
    }
  } else {
    if (cleanWidth > 1) {
      result = `<video src=\"${cleanScr}\" width=\"${cleanWidth}\"></video>`;
    } else {
      result = `<video src=\"${cleanScr}\"></video>`;
    }
  }
  return result;
}
...
function normalizeCoord(value) {
  ...
  // Check if the coordinate format is "[lng, lat]"
  if (value.includes('[') && value.includes(']')) {
    const parts = value.substring(1, value.length - 1).split(',');
    lat = parseFloat(parts[1]);
    lng = parseFloat(parts[0]);
  }
  // Check if the coordinate format is "lat, lng"
  ...
  // Format the coordinate as "(lat, lng)"
  return `(${lat}, ${lng})`;
}
...
function formatCoords(...values) {
  ...
  for (let i = 0; i < values.length; i++) {
    try {
      const normalizedCoord = normalizeCoord(values[i]);
...
function generateLicenseLink(licenseCode, targetBlank = false) {
  ...
  const licenseUrl =
    licenseCode && licenseCode.startsWith('CC-')
      ? `https://creativecommons.org/licenses/${licenseCode.toLowerCase().substring(3)}/4.0/`
      : 'https://choosealicense.com/no-permission/';

  const linkTarget = targetBlank ? ' target="_blank"' : '';

  return `<a href="${licenseUrl}"${linkTarget}>${
...
function pureBool(value) {
  ...
  const falseValues = ['no', 'n', 'non', 'n', 'f', 'false', 'faux', '0'];

  if (trueValues.includes(lowercasedValue) || Number(value) > 0) {
    return true;
  } else if (falseValues.includes(lowercasedValue) || Number(value) < 0) {
    return false;
  } else {
...
function every(...args) {
  try {
    const boolValues = args.map((arg) => pureBool(arg));
...
function any(...args) {
  try {
    const boolValues = args.map((arg) => pureBool(arg));
    return boolValues.some((value) => value === true);
  } catch (error) {
    return false;
  }
}
...
function buildUrl(query, order, count, license) {
  ...
  if (count < 1 || count > 50) {
    throw new Error('Invalid count value. Count must be between 1 and 50.');
  }
  ...
  if (!validLicenses.includes(license)) {
    throw new Error(
      'Invalid license value. License must be one of: none, any, cc-by, cc-by-nc, cc-by-sa, cc-by-nd, cc-by-nc-sa, cc-by-nc-nd'
    );
  }
