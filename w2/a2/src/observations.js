...
function observationSummary(data) {
  for (let i = 0; i < data.results.length; i++) {
    console.log(
      `ID-${data.results[i].id} "${data.results[i].species_guess}" (DATE: ${data.results[i].observed_on_details.date})`
    );
  }
}
...
function observationsByGeoPrivacy(data, geoPrivacy) {
  ...
  if (
    ...
    for (var observation of data.results) {
      if (!geoPrivacy) {
        if (observation.geoprivacy == geoPrivacy) {
          newArr.push(observation);
        }
      } else if (observation.geoprivacy == geoPrivacy.toLowerCase()) {
        newArr.push(observation);
      }
    }
  } else {
...
function transformObservation(original) {
  // TODO
  var modifiedLoc = original.location.split(',');
  var urlArr = original.photos.map((photo) => photo.url);

  var newObj = {
    id: original.id,
    speciesGuess: original.species_guess,
    isResearchQuality: original.quality_grade === 'research',
    coords: [parseFloat(modifiedLoc[1]), parseFloat(modifiedLoc[0])],
    photos: urlArr,
    photosCount: urlArr.length,
...
function transformObservations(data) {
  ...
  data.results.forEach((elements) => {
    newArr.push(transformObservation(elements));
  });
...
function getObservationsById(data, ...ids) {
  ...
  if (ids.length === 1) {
    let matched = data.results.find((elements) => elements.id === ids[0]);
    if (matched) {
      return matched;
    } else {
      return null;
  ...
  ids.forEach(function (id) {
    let matched = data.results.find((elements) => elements.id === id);
    if (matched) {
      newArr.push(matched);
    }
  });
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  ...
  if (options.gt && options.lt && options.eq) {
    filtered = data.results.filter((element) => element.positional_accuracy === options.eq);
  } else if (options.gt && options.lt) {
    filtered = data.results.filter(
      (element) =>
        element.positional_accuracy >= options.gt && element.positional_accuracy <= options.lt
    );
  } else if (options.eq) {
    filtered = data.results.filter((element) => element.positional_accuracy === options.eq);
  } else if (options.max) {
    filtered = data.results.filter((element) => element.positional_accuracy <= options.max);
  } else if (options.gt) {
    filtered = data.results.filter((element) => element.positional_accuracy >= options.gt);
  } else if (options.lt) {
    filtered = data.results.filter((element) => element.positional_accuracy <= options.lt);
  } else {
...
function getPlaces(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    var noOfIds = data.results[i].place_ids;

    var newObj = {};

    for (let j = 0; j < noOfIds.length; j++) {
      var placeId = noOfIds[j];
      var url = `https://www.inaturalist.org/observations?place_id=${placeId}`;
      newObj[placeId] = url;
    }
    result.push(newObj);
  }
...
function getUserStats(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    totalObserv += data.results[i].user.observations_count;
    totalJournal += data.results[i].user.journal_posts_count;
    totalSpecy += data.results[i].user.species_count;
  }

  let totalUser = data.results.length;

  let result = {
    count: totalUser,
    totals: {
      observations: totalObserv,
      journals: totalJournal,
      species: totalSpecy
    },
    averages: {
      observations: totalObserv / totalUser,
      journals: totalJournal / totalUser,
      species: totalSpecy / totalUser
    }
  };
...
function extractTimeZones(data) {
  ...
  for (let i in data.results) {
    if (!result.includes(data.results[i].created_time_zone)) {
      result.push(data.results[i].created_time_zone);
    }

    if (!result.includes(data.results[i].observed_time_zone)) {
      result.push(data.results[i].observed_time_zone);
    }
  }
  return result;
}
...
function extractTimeZones2(data) {
  ...
  for (let i in data.results) {
    result.add(data.results[i].created_time_zone);
    result.add(data.results[i].observed_time_zone);
  }
